...
!stealth, !unignore, !wakeup, !talk, !owner"
	...
	def __init__(self, my_pyborg, args):
		...
		self.settings.load("pyborg-im.cfg",
			{ "myname": ("The bot's nickname", "PyBorg"),
			  "realname": ("Reported 'real name'", "Pyborg"),
			  "owners": ("Owner(s) nickname", [ "OwnerNick" ]),
			  "servers": ("IRC Server to connect to (server, port [,password])", [("im.bitlbee.com", 6667)]),
			  "chans": ("Channels to auto-join", ["&bitlbee"]),
...
if __name__ == "__main__":
	...
	if "--help" in sys.argv:
		...
		print " pyborg-im.py [options]"
		print " -s   server:port"
		print " -c   channel"
		print " -n   nickname"
		print "Defaults stored in pyborg-im.cfg"
		print
		sys.exit(0)
	# start the pyborg
